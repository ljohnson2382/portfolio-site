...
Note: Verify the versions listed above against package.json (and lockfile) and update this document if they differ. The listed versions are informational and may change with package updates.
...
- Unused variables should use a leading underscore and match `/^_/u` (for example: `_unusedVar`). This allows intentional unused variables while keeping the rule strict for accidental ones.
...
1. Use `npm install <package>` (avoid shorthand commands like `npm i` in CI/automation; prefer the full command for clarity).
